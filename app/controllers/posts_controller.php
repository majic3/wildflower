...
class PostsController extends AppController {
    ...
    function admin_options($id = null) {
		...
		$short = ClassRegistry::init('Short')->check('Post', $this->data['Post']['id'], 'slug');
		if(!$short) $short = ClassRegistry::init('Short')->make('Post', $this->data['Post']['id'], 'slug');

        $this->set(compact('isDraft', 'short'));
    ...
    function index() {
		...
		$this->canonical['rel'] = '/' . Configure::read('Wildflower.blogIndex');

		//	$cats = "one|/c/one,second cat|/c/second-cat";
		$categories = '';
		foreach($sidebarCategories as $key => $var)	{
			//debug($var);
			$categories[] = "{$var['Category']['title']}|/c/{$var['Category']['slug']}";
		}
		$categories = implode(',', $categories);
		//die($categories);

        $this->set(compact('posts', 'sidebarCategories', 'categories'));
	 ...
	 * making handle ajax - hmmm normal operation seems to redirect to uuid getslug here seems to return that whereas the rest of this app is using slugs not uuid for retriving posts (I thought wf moved to uuid for posts)
     *
     * @return void
     */
