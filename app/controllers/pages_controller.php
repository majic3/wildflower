...
class PagesController extends AppController {
    ...
    function admin_options($id = null) {
		...
		$short = ClassRegistry::init('Short')->check('Page', $this->data['Page']['id']);
		if(!$short) $short = ClassRegistry::init('Short')->make('Page', $this->data['Page']['id']);
        $this->set(compact('parentPageOptions', 'short'));
    }
...
//			return $this->do404();
		...
		// Decode custom fields
				$cfarr = array();
				$cfparams = json_decode($page['Page']['custom_fields'], true);
				if($cfparams)    {
						foreach($cfparams as $k => $v)  {
								$cfarr[$v['name']] = $v['value'];
						}
						// debug($cfparams);
						$page['Page']['custom_fields'] = $cfarr;
				}
		...
		if(is_array($cfarr) && ($cfarr !== array())){
				if(array_key_exists('bdyClass', $cfarr))    {
						$this->bdyClass = $cfarr['bdyClass'];
				}

				if(array_key_exists('templateSwitch', $cfarr))    {
					$this->templateSwitch = $cfarr['templateSwitch'];
				} 

				if(array_key_exists('layout', $cfarr))    {
					$this->layout = $cfarr['layout'];
				} 
				$this->_chooseTemplate(array_key_exists('template', $cfarr) ? $cfarr['template'] : $page[$this->modelClass]['slug']);
		} else {
