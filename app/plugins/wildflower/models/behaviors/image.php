...
class ImageBehavior extends ModelBehavior {
	...
	function __getFolder(&$model, $record) {   
		$this->log($baseDir .'/'. Inflector::camelize($model->name) .'/'. $record[$model->primaryKey] . '/', 'image-behavior');
		extract($this->settings[$model->name]);
		return  $baseDir .'/'. Inflector::camelize($model->name) .'/'. $record[$model->primaryKey] . '/';
	}
	function __getFullFolder(&$model, $field) {
		$this->log(WWW_ROOT .$this->__getFolder(&$model, $field), 'image-behavior');
		extract($this->settings[$model->name]);
	...
	function __saveFile(&$model, $field, $fileData) {
		$this->log($fileData, 'image-behavior');
		extract($this->settings[$model->name]);
		...
		$this->log($folder->pwd(), 'image-behavior');
		$this->log($fields[$field], 'image-behavior');
		$this->log(json_encode(Array('tmp-name' => $fileData['tmp_name'], 'name' => $fileData['name'])), 'image-behavior') ;
	}
